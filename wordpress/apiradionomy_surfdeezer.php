<?php
// ajouter ligne 30 update_track($rq_track->ID, $track->dateofdiff);
...
require_once ('config.inc.deezer.php');
require_once ('utils.inc.deezer.php');
...
foreach ($data_radio->track as $track) {
	...
	if ($rq_track){//si titre existe bdd wordpress
		update_track($rq_track->ID, $track->dateofdiff);//incremente le nb de lecture
