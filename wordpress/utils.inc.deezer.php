...
function get_lastfm_infos($artist, $morceau,$track,$radio_current){
  global $config;
      ...
      if ($artist){ // On vérifie que l'Artiste existe dans la BDD de l'API
        ...
        $ret['deezer'] =  get_deezer_infos ($morceau);
        ...
        $date = new DateTime($track->dateofdiff);
        $ret['event_artist'] = $date->format('Y/m/d H:i:s');//heure de diffusion
        $ret['event_song'] = $date->format('Y/m/d H:i:s');//heure de diffusion
        $ret['artist_cover'] = $artistInfo['artist']['image'][3]['#text'];//cover artiste lfm
        $ret['time_song'] = time_song($track->playduration);//durée titre ms
...
function get_deezer_infos ($morceau){
  $requete = 'https://api.deezer.com/search?q=track:"';
  $query = $requete.$morceau['track']['name'].'"';
  $artistDeezerInfo = jsonQuery($query);
  foreach ($artistDeezerInfo as $infos) {
      if ($infos['name'] == $morceau['artist'] ){
       return $infos['preview'];
      }
    }
  // my_dump($artistDeezerInfo);
}
...
function insert_track($infos, $artist_bdd_id)// au cas ou nouveau titre d'un artiste existant
{
          ...
          'post_content'    => $infos['artist'].' - '.$infos['title_song'].'durée: '.$infos['time_song'].'<br><div class="song_cover"><img class="alignnone size-medium wp-image-1398" src="'.$infos['title_cover'].'" alt="'.$infos['title_song'].'" /></div>',
          'post_type'       => 'songs',
          'post_status'     => 'publish',
          ...
          wp_set_object_terms($postSongid, $postClassement, 'songs_cat');
                     ...
                     add_post_meta($postSongid,'cd_meta', $meta_value);
                     add_post_meta($postSongid,'cd_plays', array(1));//met à 1 la lecture 
                     add_post_meta($postSongid,'cd_event_date',$infos['event_song']) ;   
     ...
     //verif data bdd
      ...
      //artiste existe pas on le fait car le test à etait fait sur le titre avant.
          ...
          wp_set_object_terms($postArtistid, $abecedaire, 'artist');
           add_post_meta($postArtistid,'cd_meta', $meta_value);
           add_post_meta($postArtistid,'cd_event_date',$infos['event_artist']) ;
           add_post_meta($postArtistid,'cd_plays', array(1));//met à 1 la lecture                  
...
function update_track($id_track, $date_diff){//incremente le nb de lecture et horaire diffusion
$cd_plays = get_post_meta($id_track, $key = 'cd_plays');//incremente la lecture
$nb_plays =number_format_i18n( (int) $cd_plays);
$nb_plays++;
update_post_meta( $id_track,'cd_plays', $nb_plays);
add_post_meta($id_track, 'cd_event_date',time_song($date_diff)); //insert h diffusion
...
function time_song($song_time){//transforme le temps ms en Min:sec
  $duration = (int)$song_time; 
