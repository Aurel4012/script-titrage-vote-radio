...
if ( $_POST['action'] )
{
    ...
    $verification_si_deja_vote = mysql_query( "\r\n\tSELECT \r\n\t\r\n\tip_id_vote,\r\n\tip_adresse_ip,\r\n\t\r\n\tradio_webradios_vote.vote_id,\r\n\tradio_webradios_vote.vote_titre\r\n\t\r\n\tfrom radio_webradios_vote_ip\r\n\t\r\n\tjoin radio_webradios_vote on radio_webradios_vote.vote_id = radio_webradios_vote_ip.ip_id_vote\r\n\t\r\n\t\r\n\tWHERE `vote_titre` LIKE '".$titre."' and `ip_adresse_ip` LIKE '".$ip_adresse_ip."'  \r\n\t\r\n\t" );
    ...
    if ( $nombre_de_vote_trouve == 0 )
    {
        ...
        if ( $nombre == 0 )
        {
            if ( !( $insert = mysql_query( $Tmp_104."', '".$tp."1');" ) ) )
            {
                exit( mysql_error( ) );
            }
